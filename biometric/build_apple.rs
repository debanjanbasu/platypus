...
/// Structs for parsing Swift target info
...
/// Entry point for Apple-specific build logic.
/// This is called from build.rs only on Apple targets.
pub async fn build_for_apple() -> Result<()> {
    // This is required, as somehow cargo is ignoring this
    println!("cargo:rustc-env=DYLD_FALLBACK_LIBRARY_PATH=/usr/lib/swift");
    // Warn users to set DYLD_LIBRARY_PATH at runtime as a child build.rs cannot set
    // up env var for parent apps
    println!(
        "cargo:warning=You must set DYLD_FALLBACK_LIBRARY_PATH=/usr/lib/swift before running your app, or Swift libraries may not be found at runtime."
    );
